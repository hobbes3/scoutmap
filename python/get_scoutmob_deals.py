...
parser.add_argument( '-c', '--city'   , nargs = '*'          , default = 'all', help = "enter a list of cities for the deals (must match Scoutmob URL): default all" )
...
def get_deals_by_city( city ):
    ####################################
    # Begin website text retrieval.
    ####################################

    url = URL + "/" + city + "/past_deals"
    ...
    for i, deal in enumerate( deals, start = 1 ):
        ...
        f = deal.find( 'div', { 'class' : 'title' } ).find( 'a' )
...
debug_string( args.city, True )

if( args.city == 'all' or args.city == [] ):
    scoutmob_url = URL + "/?force=1"

    soup = BeautifulSoup.BeautifulSoup( urllib2.urlopen( scoutmob_url ) )

    div_cities = soup.find( 'div', { 'class' : 'cities' } )

    a_cities = div_cities.findAll( 'a' )

    #debug_string( a_cities, True )

    cities = []

    for a_city in a_cities:
        city = re.search( 'cities-(.+)', a_city[ 'class' ] ).group( 1 )

        deal_count += get_deals_by_city( city )
else:
